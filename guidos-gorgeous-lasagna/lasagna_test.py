...
# decode that they need to create and title their constants and functions in
# a specific way.
try:
...
# import errors
    ...
    if 'EXPECTED_BAKE_TIME' in item_name:
        ...
        raise ImportError(f'\n\nMISSING CONSTANT --> \nWe can not find or '
                          f'import the constant {item_name} in your'
        ...
        raise ImportError("\n\nMISSING FUNCTION --> In your 'lasagna.py' "
                          ...
                          f'forget to define it?') from None
...
class LasagnaTest(unittest.TestCase):
    ...
    def test_EXPECTED_BAKE_TIME(self):
        failure_msg = ('Expected a constant of EXPECTED_BAKE_TIME with a '
                       'value of 40.')
    ...
    def test_bake_time_remaining(self):
        ...
        for variant, (time, expected) in enumerate(zip(input_data, result_data)
                , start=1):
            with self.subTest(f'variation #{variant}', time=time,
                              expected=expected):
                actual_result = bake_time_remaining(time)
                failure_msg = (f'Called bake_time_remaining({time}). '
                               f'The function returned {actual_result}, but '
                               f'the tests '
                               f'expected {expected} as the remaining bake '
                               f'time.')
    ...
    def test_preparation_time_in_minutes(self):
        ...
        for variant, (layers, expected) in enumerate(
            ...
            with self.subTest(f'variation #{variant}', layers=layers,
                              expected=expected):
    ...
    def test_docstrings_were_written(self):
        ...
        """
        functions = [bake_time_remaining, preparation_time_in_minutes,
                     elapsed_time_in_minutes]
        ...
        for variant, function in enumerate(functions, start=1):
            with self.subTest(f'variation #{variant}', function=function):
                ...
                failure_msg = (
                    f'Called {function.__name__}.__doc__. {actual_result} was '
                    f'returned, '
                    f'but the tests expected a docstring for the '
